...
export const schema = yup.object().shape({
    ...
    productCategory: yup.mixed().test("productCategory", "انتخاب دسته بندی الزامی است", (value) => { return value?.length }),
...
export const editSchema = (editImages)=> yup.object().shape({
    name: yup
        .string()
        .required("این فیلد الزامی است!")
        .test("english", "فقط نام انگلیسی مجاز است", (value) => { return /^[a-zA-Z\s-]*$/.test(value); }),
    ...
    image: yup.mixed().test("fileNessesary", "بارگزاری تصویر الزامی است", (value) => { return value?.length || editImages.length }),
    //.test("fileSize", "حجم فایل بالاتر از حد مجاز است!", (value) => {
    //    if (value.length)
    //        return value[0].size <= 100000
    //}),
